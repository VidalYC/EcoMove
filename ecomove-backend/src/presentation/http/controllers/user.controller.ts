// src/controllers/UsuarioController.ts - ARREGLADO PARA FORMATO ORIGINAL
...
export class UsuarioController {
  // Registrar nuevo usuario - FORMATO ORIGINAL RESTAURADO
  static async register(req: Request, res: Response): Promise<void> {
    try {
      const { nombre, correo, documento, telefono, password, role } = req.body;

      // Validar campos requeridos (formato original)
      if (!nombre || !correo || !documento || !password) {
        res.status(400).json({
          success: false,
          message: 'Faltan campos obligatorios: nombre, correo, documento, password'
        });
        return;
      }
      ...
      // Crear usuario con campos originales
  ...
  // Login de usuario - FORMATO ORIGINAL RESTAURADO
  static async login(req: Request, res: Response): Promise<void> {
    try {
      const { correo, password } = req.body;

      // Validar campos (formato original)
      if (!correo || !password) {
        res.status(400).json({
          success: false,
